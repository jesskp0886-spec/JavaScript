...
for (let A: number = 85461; A > 0; A = (A - digit1) / 10) {
    digit1 = A % 10;
    if (digit1 > max) {
        max = digit1;
    }
...
for (let b: number = 85461; b > 0; b = (b - digit3) / 10) {
    digit3 = b % 10;
    if (digit3 < small) {
        small = digit3;
    }
...
for(let org:number = n; org > 0; org = (org - d) / 10){
    d = org % 10;
    rev = (rev * 10) + d;
}
...
for (let num: number = original; num > 0; num = (num - digit2) / 10) {
    digit2 = num % 10;
